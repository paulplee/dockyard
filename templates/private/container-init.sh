#!/bin/bash
# =============================================================================
# dockyard / private — Container initialization
...
if [ ! -f "${SSH_KEY}" ]; then
    ...
    ssh-keygen -t ed25519 -f "${SSH_KEY}" -N "" \
        -C "private@${CONTAINER_NAME:-$(hostname)}"
...
# ── Private app data directory permissions ─────────────────────────────────
PRIVATE_DATA_DIR="/home/${AGENT_USER}/.${CONTAINER_NAME}"
if [ -d "${PRIVATE_DATA_DIR}" ]; then
    chown -R ${AGENT_USER}:${AGENT_USER} "${PRIVATE_DATA_DIR}"
    chmod -R 750 "${PRIVATE_DATA_DIR}"
fi

# ── Private app service startup ─────────────────────────────────────────────
# If your private app ships a systemd user service, uncomment and adapt this
# block. Place the .service file in the image (in the "Private app installation"
# section of the Dockerfile) then install + enable it here.
#
